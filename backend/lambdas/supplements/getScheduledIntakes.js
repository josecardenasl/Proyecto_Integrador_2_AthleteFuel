  ...
  try {
    ...
    const result = await dynamoDb.send(new ScanCommand({
      ...
      ExpressionAttributeValues: {
        ":userId": decoded.id,
        ":prefix": "supp_schedule_"
      }
    }));

    const schedules = (result.Items || []).sort((a, b) =>
    ...
    return { statusCode: 500, body: JSON.stringify({ message: "Error retrieving supplement schedules", error: error.message }) };
  }
};
